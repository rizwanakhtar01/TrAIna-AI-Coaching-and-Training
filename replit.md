...
- **Amazon Connect Evaluation Integration (Prototype)**: Full evaluation coaching flow — Admin "Evaluation Forms" tab manages standalone scorecards (Form → Sections → Questions, each with numeric/pass-fail scoring) → Team modal links a form per team → Contact Review cards show collapsible section-by-section quality panel (form name, section avg badges, question-level scores with numeric progress bars or Pass/Fail badges, AI evaluation badge) → Challenge Patterns tab shows evaluation-triggered patterns (CP011 Empathy Deficit, CP012 FCR Failures) with purple badge and evaluation source detail card in pattern detail view → Coaching plans show "Evaluation-triggered" purple badge with evaluation context throughout the coaching flow → Agent widget surfaces low evaluation scores in Areas to Improve and Daily Focus
