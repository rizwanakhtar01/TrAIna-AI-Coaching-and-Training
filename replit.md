...
- **Amazon Connect Evaluation Integration (Prototype)**: End-to-end evaluation coaching flow — Admin "Evaluation Forms" tab configures quality criteria per team (enable/disable toggle, CRUD criteria with name/description/max score/weight, weight distribution indicator) → Contact Review cards show collapsible "Quality Evaluation" panel with per-criterion scores and AI evaluation badge → Coaching plans show "Evaluation-triggered" purple badge with evaluation context (criterion name, score, contact count) for plans auto-generated from evaluation data
